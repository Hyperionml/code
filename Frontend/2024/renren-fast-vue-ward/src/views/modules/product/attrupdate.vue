...
export default {
  ...
  methods: {
    ...
    getSpuBaseAttrs() {
      this.$http({
        // 这里的路径写死了，本来是：`/product/attr/base/listforspu/${this.spuId}`，但是由于前端的spuId是1646442380347408400，是不对的(导致该问题的原因是数字太长了)，然后前端就是根据前端页面中的错误的spu1646442380347408400发送请求的，所以有错误
    ...
    submitSpuAttrs() {
      ...
      this.$confirm("修改商品规格信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http({
            // 这里的路径写死了，本来是：`/product/attr/update/${this.spuId}`，但是由于前端的spuId是1646442380347408400，是不对的(导致该问题的原因是数字太长了)，然后前端就是根据前端页面中的错误的spu1646442380347408400发送请求的，所以有错误
            url: this.$http.adornUrl(`/product/attr/update/1646442380347408386`),
            method: "post",
